...
<title>QLUX APEX - On-Chain Data Exchange</title>
...
.swipe-btn { position: absolute; left: 4px; top: 4px; width: 67px; height: 67px; background: #f59e0b; border-radius: 14px; display: flex; align-items: center; justify-content: center; color: #000; font-weight: bold; cursor: pointer; z-index: 10; }
...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="card text-center">
        ...
        <p class="text-sm text-slate-400">リアルタイム・オンチェーンデータ取引 & サブスクリプション</p>
    </div>

    <div class="card space-y-4">
        <h2 class="text-lg font-bold text-amber-400">Teranode ライブデータストリーム</h2>
        <div class="bg-black/80 border border-cyan-500/30 rounded-xl p-4 font-mono text-xs space-y-2">
            <div class="text-cyan-400 font-bold">[STATUS: LIVE & SECURE]</div>
            <div class="text-slate-300">アクティブノード: <span class="text-amber-400">14,892 Nodes</span></div>
            <div class="text-slate-300">トランザクション速度: <span class="text-emerald-400">1,420,000 TPS</span></div>
        </div>

        <div>
            <label class="block text-xs font-bold text-amber-400 mb-1">ユーザー / メンバーID</label>
            <input type="text" id="user-handle" value="$qlux_member" class="w-full bg-black border border-white/20 rounded-lg px-3 py-2 text-white font-mono text-sm">
        </div>

        <div id="terminal" class="bg-black border border-cyan-500/50 rounded-xl p-4 font-mono text-xs hidden">
            <div class="text-cyan-400 font-bold mb-1">データアンロック完了</div>
            <div id="terminal-body" class="text-slate-300 break-all"></div>
        </div>
    </div>

    <div class="card space-y-4">
        <h3 class="text-md font-bold text-white">1スワイプ即時決済 (15 Sats)</h3>
        <div class="swipe-box" id="swipe-container">
            <div class="swipe-text" id="swipe-text">SWIPE RIGHT TO PAY &rarr;</div>
...
async function pay() {
    ...
    body.innerHTML = 'ナノペイメントをネットワークへブロードキャスト中...';
    ...
    try {
        ...
        setTimeout(() => {
            ...
            btn.style.transform = 'translateX(0px)';
            text.style.opacity = '1';
            body.innerHTML = 'ID: ' + data.handle + '<br>金額: ' + data.sats + ' SATS<br>TXID: ' + data.txid + '<br><b class="text-emerald-400">[取引成功] データストリーム解放。</b>';
        }, 400);
    } catch (e) {
        body.innerHTML = '決済通信エラーが発生しました。';
    }
}
</script>
</body>
